...
var nbRandomPoints = 10;
var nbAnts = 1;
var antVelocity = 0.0005;
var textMesh = true;
var nbCity = 10;
...
if (textMesh) {
    range(0, points.length - nbRandomPoints).forEach(function(pt){
        var directLink = forcedEdges.get(pt);
        var textEdge = createEdge(points[pt], points[directLink]);
        edges.push(textEdge);
...
function addToNextEdges(point, cell, nexts, nextEdgesMap) {

    if(nextEdgesMap.has(point))
    {
        var existingNextEdges = nextEdgesMap.get(point);
        ...
        for (var i = 0; i < existingNextEdges.length; i++)
        {
            ...
            if (cell != undefined && cell.indexOf(getOtherPoint(existingNextEdges[i], point)) == -1)
...
shell.on("render", function() {
    ...
    for(var i=0; i<points.length; ++i) {
        ...
        context.fill();
        // if (i===0){
        //     context.arc(points[i].x, points[i].y, 0.01, 0, 2*Math.PI);
        //     context.closePath();
        //     context.fill();
        // }
    }
            ...
            if (res.edgeChanged) {
                ...
                if (res.cityReached && (this.origin.id != this.lastCity) ){
                    ...
                    this.edges.forEach(function(e){
                        ...
                        if (forcedEdges.has(a.id) && forcedEdges.get(a.id) === b.id) {
                            weight *= 100;
                        }
